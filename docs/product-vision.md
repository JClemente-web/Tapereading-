...
Construir uma automação operacional para ProfitPro focada principalmente em Mini Dólar (WDO) e secundariamente em Mini Índice (WIN), com leitura de fluxo, filtragem de ruído, decisão objetiva, execução controlada, proteção automática e auditoria.

A automação deve usar o ProfitPro como ambiente de análise, execução e proteção, aproveitando recursos como Times & Trades, DOM, Footprint, Volume At Price, saldo de agressão, Editor de Estratégias, NTSL, automação e OCO quando disponíveis.
...
- operadores de WDO que usam ProfitPro;
- operadores de Tape Reading;
- scalpers profissionais;
- mesas que precisam padronizar leitura operacional;
- desenvolvedores de automação NTSL/ProfitPro;
- responsáveis por risco e auditoria operacional.
