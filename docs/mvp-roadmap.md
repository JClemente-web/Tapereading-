...
O MVP deve priorizar regras rastreáveis, automação controlada, simulação operacional, replay, proteção e auditoria antes de qualquer liberação com capital real.
...
A recomendação inicial é começar por motor de eventos, rulebook executável e simulação/replay, evoluindo para NTSL/ProfitPro somente após confirmar quais dados e funções estão disponíveis programaticamente.
